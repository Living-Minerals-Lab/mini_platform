...
"""
"""
    This code is adapted from the source code available at 
    https://github.com/LinuxCNC/simple-gcode-generators/tree/master/grid
    The author made it compatible with python 3.12.3
"""
...
class Application(Frame):
    ...
    def run_command(self):
        ...
        # self.listbox.selection_set(self.command_idx)
        # self.listbox.see(self.command_idx)
        # self.listbox.activate(self.command_idx)
        # self.listbox.selection_anchor(self.command_idx)

        self.client.emit('runCommand', self.gcode[self.command_idx])
        self.command_idx = min(self.command_idx + 1, len(self.gcode) - 1)
        # self.listbox.activate(self.command_idx)
    ...
    def createWidgets(self):
        ...
        # G17: XY plane; G21: metric units (mm); G90: absolute positioning
        self.PreambleVar.set('G17 G21 G90') 
        ...
        self.st021 = Label(self.EntryFrame, text='#X points')
        self.st021.grid(row=self.rownumber, column=2)
        self.XPointsVar = StringVar()
        self.XPointsVar.set('1')
        self.XPoints = Entry(self.EntryFrame, textvariable=self.XPointsVar ,width=10)
        self.XPoints.grid(row=self.rownumber, column=3,sticky=W)
        ...
        self.st031 = Label(self.EntryFrame, text='#Y points')
        self.st031.grid(row=self.rownumber, column=2)
        self.YPointsVar = StringVar()
        self.YPointsVar.set('1')
        self.YPoints = Entry(self.EntryFrame, textvariable=self.YPointsVar ,width=10)
        self.YPoints.grid(row=self.rownumber, column=3,sticky=W)
        ...
        self.ToFile = Button(self.EntryFrame, text='To File', command=self.SaveToFile)
        self.ToFile.grid(row=self.rownumber, column=2)

        self.SendCommand = Button(self.EntryFrame, text='Send Command', command=self.run_command)
        self.SendCommand.grid(row=self.rownumber, column=3)

        self.rownumber += 1
        # langs = ('Java', 'C#', 'C', 'C++', 'Python',
        #         'Go', 'JavaScript', 'PHP', 'Swift')

        # self.lvar = Variable(self.EntryFrame, value=self.gcode)
        ...
        self.listbox.grid(row=self.rownumber, column=0)
    ...
    def items_selected(self, event):
        ...
        if selected_indices:
            ...
            showinfo(title='Information', message=self.gcode[selected_indices[0]])

    def Change_Units(self):
        if (self.UnitVar.get() == 0) :
            #Inch 
            # self.PreambleVar.set('G17 G20 G90 G64 P0.001 M3 S3000 M7')
            self.PreambleVar.set('G17 G21 G91') 
        else :
            #MM
            # self.PreambleVar.set('G17 G21 G90 G64 P0.01 M3 S3000 M7')
            self.PreambleVar.set('G17 G21 G90') 
    ...
    def SaveToFile(self):
        with open('gen_gc.gc', 'w') as file:
            file.writelines(self.gcode)
