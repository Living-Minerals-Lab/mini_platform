...
class Z300Controller(AnalyticalDeviceController):
    ...
    def measure(self) -> None:
        ...
        In ROS, get_button_pos_multi_scale and pyautogui.click should be called separately instead.
        ...
        if not self.is_device_ready():
            ...
            # x, y = self.get_button_pos_multi_scale(self.measure_button_path)
            # pyautogui.click(x, y)
            self._process = Process(target=self.match_click_alter, args=(self.dev_status, self.measure_duration, self.measure_button_path, ))
            self._process.start()
    ...
    def _get_button_pos(self, button_template_path: str) -> tuple[int]:
        """
    ...
    def match_click_alter(self, status: Value, wait_time: float, button_template_path: str) -> None: #type: ignore
        """
        ...
        x, y = self.get_button_pos_multi_scale(button_template_path=button_template_path)
        pyautogui.click(x, y)
